...
// 페이지 제목 및 스타일 설정
$g5['title'] = 'Newsletter Subscription Management';
...
if ($action === 'unsubscribe' && $key && $email) {
    ...
    if (!verify_unsubscribe_key($key, $email)) {
        $error_message = "Invalid unsubscribe link.";
    } else {
        // 구독 취소 처리
        ...
        if ($subscribe) {
            if ($subscribe['sb_subscribe'] == 0) {
                $error_message = "This email address is already unsubscribed.";
            } else {
                // 구독 취소
                ...
                $success_message = "Subscription has been successfully cancelled.";
            }
        } else {
            $error_message = "Subscription information not found.";
        }
    }
}
// 구독 신청 처리
elseif ($action === 'subscribe' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$newsletter_agree || !$privacy_agree) {
        $error_message = "Newsletter subscription consent and personal information collection consent are required.";
        ...
        if ($subscribe) {
            if ($subscribe['sb_subscribe'] == 1) {
                ...
                $success_message = "Subscription has been successfully registered.";
            }
        } else {
            // 새로운 구독자 추가
...
<section>
    <div class="contact-container">
        <h1 class="tableAi"><span class="sound_only"><?php echo $is_unsubscribe_page ? 'Unsubscribe' : 'Subscribe'; ?></span></h1>
    ...
    <?php if ($is_unsubscribe_page): ?>
        ...
        <div style="text-align: center; padding: 20px;">
            <p style="margin-bottom: 20px;">Would you like to unsubscribe from the following email address?</p>
            ...
            <form method="get" style="display: inline;">
                ...
                <button type="submit" class="contact-submit-btn" style="background: #dc3545; margin-right: 10px;">Unsubscribe</button>
            </form>
            <a href="<?php echo G5_URL; ?>/plus/" class="contact-submit-btn" style="display: inline-block; text-decoration: none; background: #6c757d;">Cancel</a>
        </div>
        <?php else: ?>
        <div style="text-align: center; padding: 20px;">
            <a href="<?php echo G5_URL; ?>/plus/" class="contact-submit-btn" style="display: inline-block; text-decoration: none; background: #6c757d;">Go to Home</a>
        </div>
        <?php endif; ?>
    <?php else: ?>
        <!-- 구독 신청 폼 -->
        ...
        <div class="contact-form" style="border: 1px solid #ddd; margin-bottom: 50px;">
            <form method="post" action="?action=subscribe" onsubmit="return validateForm()">
                ...
                <div class="contact-form-group">
                    <label for="email">💌 Please enter the email address where you would like to receive the newsletter.</label>
                    <input type="email" id="email" name="email" 
                           value="<?php echo htmlspecialchars($member_email); ?>" 
                           placeholder="<?php echo $is_member ? 'Edit your email' : 'Enter your email'; ?>" 
                ...
                <div style="text-align: left;">
                    <div style="display: flex; align-items: flex-start; margin: 10px 0;">
                        ...
                        <label for="newsletter_agree" style="font-size: 14px; color: #666;">I agree to receive the newsletter from <?php echo $config['cf_title']; ?> via this email address.</label>
                    </div>
                </div>
                ...
                <div style="text-align: left;">
                    <div style="display: flex; align-items: flex-start; margin: 10px 0;">
                        ...
                        <label for="privacy_agree" style="font-size: 14px; color: #666;">I agree to <?php echo $config['cf_title']; ?> collecting personal information for newsletter delivery.</label>
                    </div>
                </div>

                <br>
                
                <button type="submit" class="contact-submit-btn">Subscribe</button>
            </form>
        </div>
    <?php endif; ?>
</div>
...
function validateForm() {
    ...
    if (!newsletter_agree || !privacy_agree) {
        alert('Newsletter subscription consent and personal information collection consent are required.');
        return false;
    }
