...
<section id="bo_w" class="sub-container">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
          ...
          <tr>
      <th scope="row"><label for="wr_subject_0">Year<strong class="sound_only">Required</strong></label></th>
      <td>
          <?php
          // Set the initial year and current year
          $index_year = "1982";  // The starting year
          $wr_year = date("Y");  // The current year
          $len_year = $wr_year - $index_year + 1;  // Calculate the number of years to display

          // Loop to create radio buttons for each year
          for ($i = 0; $i < $len_year; $i++) {
          ?>
              <!-- Radio button input with the year as the value -->
              <span><input type="radio" name="wr_subject" id="wr_subject_<?php echo $i ?>" value="<?php echo $wr_year; ?>" <?php if ($subject == $wr_year) { echo 'checked'; } ?>>
              <!-- Label for the radio button displaying the year -->
              <label for="wr_subject_<?php echo $i ?>" class="ctt_013_label"><?php echo $wr_year; ?></label></span>
    ...
    <td class="wr_content">
        ...
        <ul>
            ...
            $spec_title = explode('//', $content);

            // Loop to create list items with input fields for each content part
            for ($i = 0; $i < count($spec_title); $i++) {
            ?>
                <li>
                    <!-- Input field for content with value from $spec_title -->
                    <input type="text" class="wr_content" value="<?=htmlspecialchars($spec_title[$i])?>" size="100" placeholder='Input'>
                    <!-- Delete button -->
    ...
    <!-- Submit button to complete the writing -->
    <input type="submit" value="Complete Writing" id="btn_submit" accesskey="s" class="btn_submit btn">
    <!-- Cancel button to return to the board -->
    ...
    <script>
          cont_val();
          $(document).on("click", ".wr_content .add", function() {
              $(".wr_content ul").append("<li><input type='text' class='wr_content' size='100' placeholder='입력'><a class='del'>삭제</a></li>");
              cont_val();
          })

          $(document).on("click", ".wr_content li .del", function() {
              $(this).parent().remove();
              cont_val();
          })

          $(document).on("keyup", ".wr_content li input", function() {
              cont_val();
          })

          function cont_val() {
              var val1 = "";
              for(var i=1; i<=$(".wr_content ul li").length; i++) {
                  if(i>1) {
                      val1 += "//";
                  }
                  val1 += $(".wr_content ul li:nth-of-type("+i+") .wr_content").val();
              }
              $(".wr_content input[name=wr_content]").val(val1);
          }
      </script>
