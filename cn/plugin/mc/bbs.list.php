...
if(G53){
    include dirname(__FILE__)."/bba.list.53.php";
    return;
}
/** @var string $bo_table */
/** @var array<string, string> $board */
$mc_add_query = mc_board($bo_table)->getAddQueryString($_GET);


if (!empty($qstr)) {
    $qstr = '&amp;' . $mc_add_query. $qstr;
} else {
    $qstr = $mc_add_query;
}

function mc_get_sql_search_old($search_ca_name, $search_field, $search_text, $search_operator = 'and')
{
    global $g5, $mc_search;
...
if ($board['bo_use_category']) {
    $is_category = true;
    $category_href = get_pretty_url($bo_table, '', $mc_add_query);
    ...
    for ($i = 0; $i < count($categories); $i++) {
        ...
        $category_option .= '<li><a href="' . (get_pretty_url($bo_table, '', 'sca=' . urlencode($category) . '&amp;' . $mc_add_query)) . '"';
        $category_msg = '';
...
if ($sca || $stx || $stx === '0' || $mc_search) {     //검색이면
    $is_search_bbs = true;      //검색구분변수 true 지정
    $sql_search = mc_get_sql_search($sca, $sfl, $stx, $sop);
...
//2020-02-11
//$qstr = ($config['cf_bbs_rewrite']? ltrim($qstr,'?'):$qstr);
