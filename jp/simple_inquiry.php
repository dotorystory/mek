...
if(isset($_POST['simple_email'])) {
    ...
    $email_message = "会社名 : ".clean_string($name)."<br><br>";
    $email_message .= "メールアドレス : ".clean_string($email_from)."<br><br>";
    $email_message .= "電話番号 : ".clean_string($phone)."<br><br>";
    $email_message .= "お問い合わせ内容 : ".clean_string(nl2br($message))."<br><br>";
...
<div id="simple_inquiry_layer" class="simple_inquiry_layer">
    <div class="simple_inquiry_content">
        ...
        <form name="simple_inquiry_form" method="post" action="/jp/simple_inquiry.php" class="simple_inquiry_form">
            <div class="simple_inquiry_fields">
                <div class="field_group">
                    <label for="simple_name" class="required">会社名</label>
                    <input type="text" name="simple_name" id="simple_name" required placeholder="相談参照用の会社名を入力してください。（非公開）">
                </div>
                
                <div class="field_group">
                    <label for="simple_email" class="required">メールアドレス</label>
                    <input type="email" name="simple_email" id="simple_email" required placeholder="連絡可能なメールアドレスを入力してください。">
                </div>
                
                <div class="field_group">
                    <label for="simple_phone">電話番号</label>
                    <input type="tel" name="simple_phone" id="simple_phone" placeholder="担当者携帯電話/電話番号等を入力してください。">
                </div>
                
                <div class="field_group">
                    <label for="simple_message" class="required">お問い合わせ内容</label>
                    <textarea name="simple_message" id="simple_message" required placeholder="お問い合わせ内容を入力してください。（最大500文字）" rows="2"></textarea>
                </div>
            ...
            <div class="simple_inquiry_footer">
                <button type="submit" class="simple_inquiry_submit">送信</button>
            </div>
        </form>
    </div>
</div>
...
document.querySelector('.simple_inquiry_form').addEventListener('submit', function(e) {
    ...
    fetch('/jp/simple_inquiry.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.text())
    .then(data => {
        // サーバーレスポンスを確認して成功/失敗を処理
        if(data.includes('自動登録防止番号が間違っています')) {
            alert('自動登録防止番号が間違っています。\n再度入力してください。');
            // キャプチャの更新 (kcaptchaの場合)
        ...
        } else if(data.includes('お問い合わせありがとうございます')) {
            alert('お問い合わせありがとうございます。\nできるだけ早くご返信いたします。');
            closeSimpleInquiry();
            this.reset();
        } else {
