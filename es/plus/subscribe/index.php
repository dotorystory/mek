...
// 페이지 제목 및 스타일 설정
$g5['title'] = 'Gestión de Suscripción al Boletín';
...
if ($action === 'unsubscribe' && $key && $email) {
    ...
    if (!verify_unsubscribe_key($key, $email)) {
        $error_message = "Enlace de cancelación de suscripción no válido.";
    } else {
        // 구독 취소 처리
        ...
        if ($subscribe) {
            if ($subscribe['sb_subscribe'] == 0) {
                $error_message = "Esta dirección de correo electrónico ya está cancelada.";
            } else {
                // 구독 취소
                ...
                $success_message = "La suscripción se ha cancelado exitosamente.";
            }
        } else {
            $error_message = "Información de suscripción no encontrada.";
        }
    }
}
// 구독 신청 처리
elseif ($action === 'subscribe' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$newsletter_agree || !$privacy_agree) {
        $error_message = "Se requiere el consentimiento de suscripción al boletín y el consentimiento de recopilación de información personal.";
        ...
        if ($subscribe) {
            if ($subscribe['sb_subscribe'] == 1) {
                $error_message = "Esta dirección de correo electrónico ya está suscrita.";
                ...
                $success_message = "La suscripción se ha registrado exitosamente.";
            }
        } else {
            // 새로운 구독자 추가
...
<section>
    <div class="contact-container">
        <h1 class="tableAi"><span class="sound_only"><?php echo $is_unsubscribe_page ? 'Cancelar Suscripción' : 'Suscribirse'; ?></span></h1>
    ...
    <?php if ($is_unsubscribe_page): ?>
        ...
        <div style="text-align: center; padding: 20px;">
            <p style="margin-bottom: 20px;">¿Desea cancelar la suscripción de la siguiente dirección de correo electrónico?</p>
            ...
            <form method="get" style="display: inline;">
                ...
                <button type="submit" class="contact-submit-btn" style="background: #dc3545; margin-right: 10px;">Cancelar Suscripción</button>
            </form>
            <a href="<?php echo G5_URL; ?>/plus/" class="contact-submit-btn" style="display: inline-block; text-decoration: none; background: #6c757d;">Cancelar</a>
        </div>
        <?php else: ?>
        <div style="text-align: center; padding: 20px;">
            <a href="<?php echo G5_URL; ?>/plus/" class="contact-submit-btn" style="display: inline-block; text-decoration: none; background: #6c757d;">Ir al Inicio</a>
        </div>
        <?php endif; ?>
    <?php else: ?>
        <!-- 구독 신청 폼 -->
        ...
        <div class="contact-form" style="border: 1px solid #ddd; margin-bottom: 50px;">
            <form method="post" action="?action=subscribe" onsubmit="return validateForm()">
                ...
                <div class="contact-form-group">
                    <label for="email">💌 Por favor, ingrese la dirección de correo electrónico donde desea recibir el boletín.</label>
                    <input type="email" id="email" name="email" 
                           value="<?php echo htmlspecialchars($member_email); ?>" 
                           placeholder="<?php echo $is_member ? 'Edite su correo electrónico' : 'Ingrese su correo electrónico'; ?>" 
                ...
                <div style="text-align: left;">
                    <div style="display: flex; align-items: flex-start; margin: 10px 0;">
                        ...
                        <label for="newsletter_agree" style="font-size: 14px; color: #666;">Acepto recibir el boletín de <?php echo $config['cf_title']; ?> a través de esta dirección de correo electrónico.</label>
                    </div>
                </div>
                ...
                <div style="text-align: left;">
                    <div style="display: flex; align-items: flex-start; margin: 10px 0;">
                        ...
                        <label for="privacy_agree" style="font-size: 14px; color: #666;">Acepto que <?php echo $config['cf_title']; ?> recopile información personal para la entrega del boletín.</label>
                    </div>
                </div>

                <br>
                
                <button type="submit" class="contact-submit-btn">Suscribirse</button>
            </form>
        </div>
    <?php endif; ?>
</div>
...
function validateForm() {
    ...
    if (!newsletter_agree || !privacy_agree) {
        alert('Se requiere el consentimiento de suscripción al boletín y el consentimiento de recopilación de información personal.');
        return false;
    }
