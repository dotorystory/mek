...
if(isset($_POST['simple_email'])) {
    ...
    function died($error) {
        echo "<script> alert('El envío del correo electrónico falló.');";
        echo "history.go(-1);";
        echo "</script>";
        die();
    }
    ...
    if(!isset($_POST['simple_name']) ||
        ...
        died('Lo sentimos.\nHay un problema con el formulario enviado.\nPor favor, verifique el formulario nuevamente.');
    }

    // Captcha verification
    if(!chk_captcha()) {
        died('El código de seguridad es incorrecto.');
    }
    ...
    $email_subject = "[Consulta Simple] Consulta de ".$name;
    ...
    $email_message = "Nombre de la Empresa : ".clean_string($name)."<br><br>";
    $email_message .= "Correo Electrónico : ".clean_string($email_from)."<br><br>";
    $email_message .= "Teléfono : ".clean_string($phone)."<br><br>";
    $email_message .= "Mensaje : ".clean_string(nl2br($message))."<br><br>";
...
<div id="simple_inquiry_layer" class="simple_inquiry_layer">
    <div class="simple_inquiry_content">
        ...
        <form name="simple_inquiry_form" method="post" action="/es/simple_inquiry.php" class="simple_inquiry_form">
            <div class="simple_inquiry_fields">
                <div class="field_group">
                    <label for="simple_name" class="required">Nombre de la Empresa</label>
                    <input type="text" name="simple_name" id="simple_name" required placeholder="Ingrese el nombre de su empresa (para referencia, confidencial)">
                </div>
                
                <div class="field_group">
                    <label for="simple_email" class="required">Correo Electrónico</label>
                    <input type="email" name="simple_email" id="simple_email" required placeholder="Ingrese una dirección de correo electrónico accesible">
                </div>
                
                <div class="field_group">
                    <label for="simple_phone">Teléfono</label>
                    <input type="tel" name="simple_phone" id="simple_phone" placeholder="Ingrese su número de teléfono móvil/fijo">
                </div>
                
                <div class="field_group">
                    <label for="simple_message" class="required">Mensaje</label>
                    <textarea name="simple_message" id="simple_message" required placeholder="Ingrese su consulta (máximo 500 caracteres)" rows="2"></textarea>
                </div>
                ...
                <div class="field_group">
                    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap;">
                        <div style="display: flex; align-items: center;">
                            ...
                            <label for="simple_agree" style="margin: 0; display: inline;">Acepto la 'Política de Privacidad'.</label>
                        </div>
                        <a href="javascript:void(0);" onclick="togglePrivacyContent()" style="color: #1a4691; text-decoration: underline; font-size: 13px; margin-left: 10px; cursor: pointer;">Política de Privacidad</a>
                    </div>
            ...
            <div class="simple_inquiry_footer">
                <button type="submit" class="simple_inquiry_submit">Enviar Consulta</button>
            </div>
        </form>
    </div>
</div>
...
document.querySelector('.simple_inquiry_form').addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        if(data.includes('El código de seguridad es incorrecto')) {
        ...
        } else if(data.includes('Gracias por su consulta')) {
            alert('Gracias por su consulta.\nResponderemos lo antes posible.');
            closeSimpleInquiry();
            this.reset();
        } else {
