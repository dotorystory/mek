...
if(isset($_POST['email'])) {
  ...
  if (!chk_captcha()) {
    echo "<script> alert('Der Sicherheitscode ist falsch.');";
    echo "history.go(-1);";
    echo "</script>";
    die();
  }
  ...
  $email_subject = "[Dickenmessgerät Produktanfrage]";
  ...
  $email_message .= "APC-Anwendung : ".(!empty($apc) ? clean_string($apc) : 'Keine')."<br><br>"; //선택항목
  $email_message .= "Linienstatus : ".(!empty($lineStatus) ? clean_string($lineStatus) : 'Keine')."<br><br>"; //선택항목
