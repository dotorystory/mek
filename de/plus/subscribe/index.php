...
// 페이지 제목 및 스타일 설정
$g5['title'] = 'Newsletter-Abonnement verwalten';
...
if ($action === 'unsubscribe' && $key && $email) {
    ...
    if (!verify_unsubscribe_key($key, $email)) {
        $error_message = "Ungültiger Link zum Abmelden.";
    } else {
        // 구독 취소 처리
        ...
        if ($subscribe) {
            if ($subscribe['sb_subscribe'] == 0) {
                $error_message = "Diese E-Mail-Adresse ist bereits abgemeldet.";
            } else {
                // 구독 취소
                ...
                $success_message = "Abonnement wurde erfolgreich gekündigt.";
            }
        } else {
            $error_message = "Abonnement-Informationen nicht gefunden.";
        }
    }
}
// 구독 신청 처리
elseif ($action === 'subscribe' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$newsletter_agree || !$privacy_agree) {
        $error_message = "Die Zustimmung zum Erhalt des Newsletters und zur Datenerfassung ist erforderlich.";
        ...
        if ($subscribe) {
            if ($subscribe['sb_subscribe'] == 1) {
                $error_message = "Diese E-Mail-Adresse ist bereits angemeldet.";
                ...
                $success_message = "Abonnement wurde erfolgreich angemeldet.";
            }
        } else {
            // 새로운 구독자 추가
...
<section>
    <div class="contact-container">
        <h1 class="tableAi"><span class="sound_only"><?php echo $is_unsubscribe_page ? 'Abonnement kündigen' : 'Abonnieren'; ?></span></h1>
    ...
    <?php if ($is_unsubscribe_page): ?>
        ...
        <div style="text-align: center; padding: 20px;">
            <p style="margin-bottom: 20px;">Möchten Sie das Abonnement für die folgende E-Mail-Adresse kündigen?</p>
            ...
            <form method="get" style="display: inline;">
                ...
                <button type="submit" class="contact-submit-btn" style="background: #dc3545; margin-right: 10px;">Abonnement kündigen</button>
            </form>
            <a href="<?php echo G5_URL; ?>/plus/" class="contact-submit-btn" style="display: inline-block; text-decoration: none; background: #6c757d;">Abbrechen</a>
        </div>
        <?php else: ?>
        <div style="text-align: center; padding: 20px;">
            <a href="<?php echo G5_URL; ?>/plus/" class="contact-submit-btn" style="display: inline-block; text-decoration: none; background: #6c757d;">Zur Startseite</a>
        </div>
        <?php endif; ?>
    <?php else: ?>
        <!-- 구독 신청 폼 -->
        ...
        <div class="contact-form" style="border: 1px solid #ddd; margin-bottom: 50px;">
            <form method="post" action="?action=subscribe" onsubmit="return validateForm()">
                ...
                <div class="contact-form-group">
                    <label for="email">💌 Bitte geben Sie die E-Mail-Adresse ein, an die Sie den Newsletter erhalten möchten.</label>
                    <input type="email" id="email" name="email" 
                           value="<?php echo htmlspecialchars($member_email); ?>" 
                           placeholder="<?php echo $is_member ? 'E-Mail-Adresse bearbeiten' : 'E-Mail-Adresse eingeben'; ?>" 
                ...
                <div style="text-align: left;">
                    <div style="display: flex; align-items: flex-start; margin: 10px 0;">
                        ...
                        <label for="newsletter_agree" style="font-size: 14px; color: #666;">Ich stimme dem Erhalt des Newsletters von <?php echo $config['cf_title']; ?> über diese E-Mail-Adresse zu.</label>
                    </div>
                </div>
                ...
                <div style="text-align: left;">
                    <div style="display: flex; align-items: flex-start; margin: 10px 0;">
                        ...
                        <label for="privacy_agree" style="font-size: 14px; color: #666;">Ich stimme der Datenerfassung von <?php echo $config['cf_title']; ?> für den Newsletter-Versand zu.</label>
                    </div>
                </div>

                <br>
                
                <button type="submit" class="contact-submit-btn">Abonnieren</button>
            </form>
        </div>
    <?php endif; ?>
</div>
...
function validateForm() {
    ...
    if (!newsletter_agree || !privacy_agree) {
        alert('Die Zustimmung zum Erhalt des Newsletters und zur Datenerfassung ist erforderlich.');
        return false;
    }
