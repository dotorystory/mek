...
if(isset($_POST['simple_email'])) {
    ...
    function died($error) {
        echo "<script> alert('E-Mail-Versand fehlgeschlagen.');";
        echo "history.go(-1);";
        echo "</script>";
        die();
    }
    ...
    if(!isset($_POST['simple_name']) ||
        ...
        died('Entschuldigung.\nEs gibt ein Problem mit dem übermittelten Formular.\nBitte überprüfen Sie das Formular erneut.');
    }

    // Captcha verification
    if(!chk_captcha()) {
        died('Der Sicherheitscode ist falsch.');
    }
    ...
    $email_message = "Firmenname : ".clean_string($name)."<br><br>";
    $email_message .= "E-Mail : ".clean_string($email_from)."<br><br>";
    $email_message .= "Telefon : ".clean_string($phone)."<br><br>";
    $email_message .= "Nachricht : ".clean_string(nl2br($message))."<br><br>";
...
<div id="simple_inquiry_layer" class="simple_inquiry_layer">
    <div class="simple_inquiry_content">
        ...
        <form name="simple_inquiry_form" method="post" action="/de/simple_inquiry.php" class="simple_inquiry_form">
            <div class="simple_inquiry_fields">
                <div class="field_group">
                    <label for="simple_name" class="required">Firmenname</label>
                    <input type="text" name="simple_name" id="simple_name" required placeholder="Geben Sie Ihren Firmennamen ein (vertraulich)">
                </div>
                
                <div class="field_group">
                    <label for="simple_email" class="required">E-Mail</label>
                    <input type="email" name="simple_email" id="simple_email" required placeholder="Geben Sie eine erreichbare E-Mail-Adresse ein">
                </div>
                
                <div class="field_group">
                    <label for="simple_phone">Telefon</label>
                    <input type="tel" name="simple_phone" id="simple_phone" placeholder="Geben Sie Ihre Telefonnummer ein">
                </div>
                
                <div class="field_group">
                    <label for="simple_message" class="required">Nachricht</label>
                    <textarea name="simple_message" id="simple_message" required placeholder="Geben Sie Ihre Anfrage ein (max. 500 Zeichen)" rows="2"></textarea>
                </div>
            ...
            <div class="simple_inquiry_footer">
                <button type="submit" class="simple_inquiry_submit">Anfrage senden</button>
            </div>
        </form>
    </div>
</div>
...
document.querySelector('.simple_inquiry_form').addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        if(data.includes('Der Sicherheitscode ist falsch')) {
        ...
        } else if(data.includes('Vielen Dank für Ihre Anfrage')) {
            alert('Vielen Dank für Ihre Anfrage.\nWir werden so schnell wie möglich antworten.');
            closeSimpleInquiry();
            this.reset();
        } else {
...
// Datenschutzerklärung-Inhalt umschalten
