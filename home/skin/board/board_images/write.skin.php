...
<section id="bo_w">
    ...
    <div class="bo_w_link write_div">
        <label for="wr_link1"><i class="fa fa-link" aria-hidden="true"></i><span class="sound_only"> 링크  #</span></label>
        <input type="text" name="wr_link1" value="<?php if($w=="u"){echo$write['wr_link1'];} ?>" id="wr_link1" class="frm_input full_input" size="50" placeholder="http 부터 시작되는 링크를 넣어주세요(링크가 없으면 배너를 눌러도 반응하지 않습니다.)">
    </div>
    ...
    <?php } ?>
	<div class="infoTxt">
		현재 설정된 이미지 최적 사이즈는 : 가로 <b>955</b> px /  세로 <b>418</b> px 입니다. <br>
	</div>
    ...
    <script>
	/* 글쓰기 기본내용 대신 작성날짜 시간 입력됨 by WEBsiting.co.kr */
	$( '#wr_content' ).attr( 'value', '수정 [ <?php echo  date("20y-m-d H:i"); ?> ]' );
	$( '#wr_content' ).css('display','none');
    <?php if($write_min || $write_max) { ?>
    // 글자수 제한
