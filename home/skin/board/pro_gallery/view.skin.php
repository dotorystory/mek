...
// 360도 뷰: wr_8 있을 때만 folderName 설정 후 KeyShotXR 초기화 스크립트 include
if (!empty($view['wr_8'])) {
	$v8_safe = preg_replace('/[^0-9a-zA-Z_.-]/', '', $view['wr_8']);
?>
<script>window.folderName = "../../img360/<?php echo htmlspecialchars($v8_safe, ENT_QUOTES); ?>";</script>
<?php
...
<article id="bo_v" style="width:<?php echo $width; ?>">
    <header>
        <h2 id="bo_v_title">
            ...
            <span class="bo_v_tit">
            <?php
            echo cut_str(get_text($view['wr_7']), 70); // 글제목 출력
            ?></span>
        </h2>
    </header>
            ...
            <li><a href="<?php echo $list_href ?>" class="btn_b01 btn" title="목록"><i class="ri-menu-line"></i><span class="sound_only">목록</span></a></li>
                ...
                <li>
                  ...
                  <ul class="more_opt is_view_btn">
                      <?php if ($update_href) { ?><li><a href="<?php echo $update_href ?>">수정<i class="ri-edit-line"></i></a></li><?php } ?>
                      <?php if ($delete_href) { ?><li><a href="<?php echo $delete_href ?>" onclick="del(this.href); return false;">삭제<i class="ri-delete-bin-line"></i></a></li><?php } ?>
                      <?php if ($copy_href) { ?><li><a href="<?php echo $copy_href ?>" onclick="board_move(this.href); return false;">복사<i class="ri-file-copy-line"></i></a></li><?php } ?>
                      <?php if ($move_href) { ?><li><a href="<?php echo $move_href ?>" onclick="board_move(this.href); return false;">이동<i class="ri-share-forward-box-line"></i></a></li><?php } ?>
                      <?php if ($search_href) { ?><li><a href="<?php echo $search_href ?>">검색<i class="ri-search-line"></i></a></li><?php } ?>
                  </ul>
                </li>
                <?php } ?>
              </ul>
      ...
      const callback = function(mutationsList, observer) {
          for(let mutation of mutationsList) {
              if (mutation.type === 'childList') {
                  let mcViewElement = document.querySelector(".mc-view-pro_ko");
                  if(mcViewElement) {
...
<!--3d 이미지 출력 -->
    ...
    <div id="softwear" class="sub-container">
      ...
      <div class="view_list_wrap view_list_wrap01">
        <h5>MEK 프로그램의 특징</h5>
        <ul>
          <li><img src="<?=G5_URL?>/img/vi_img02.png"><span>• 사용자 친화적 구성 - 직관적 인터페이스</span></li>
          <li><img src="<?=G5_URL?>/img/vi_img03.png"><span>• 언어 선택가능 한, 영, 중, 일</span></li>
          <li><img src="<?=G5_URL?>/img/vi_img04.png"><span>• User HMI와 측정 데이터 통신(선택사양)</span></li>
          <li><img src="<?=G5_URL?>/img/vi_img05.png"><span>• 사용자 요청에 맞춤 제작 가능 </span></li>
        </ul>
      </div>
  ...
  <tbody>
    ...
    <tr class="table_tr">
      ...
      <div id="apc" class="sub-container">
        ...
        <div class="view_list_wrap view_list_wrap04">
          <h5>MEK 프로그램의 특징</h5>
          <ul>
            <li>• 최적화된 두께 제어 알고리즘</li>
            <li>• 신속한 두께 제어</li>
            <li>• Auto Mapping: 센터볼트 기준</li>
            <li>• 매끈한 Mill Roll 외관</li>
            <li>• 수 Micron이내의 Control Gain 자동 조정</li>
            <li>• 다이볼트 온도 피드백 제어 (선택사양)</li>
            <li>• MD (Machine Direction) 제어 가능 (선택사양)</li>
          </ul>
        </div>
        <div class="view_list_wrap view_list_wrap05">
          <h5>APC적용 후 프로파일 변화</h5>
          <div class="apc_grid">
            <div class="apc_txt">
            ...
            <div class="apc_arrow">
              <p> <img src="<?=G5_URL?>/img/pro_arrow.png" alt="아래화살표"> </p>
            </div>
            <div class="apc_void">
    ...
    <div class="control sub-container">
    	<div class="khwrap">
    		<div class="button fl">
    			<?php if ($update_href) { ?><a href="<?php echo $update_href ?>" class="bt bt_b02"><i class="ri-edit-line"></i> <?php echo _("수정") ?></a><?php } ?>
    			<?php if ($delete_href) { ?><a href="<?php echo $delete_href ?>" class="bt bt_adm" onclick="del(this.href); return false;"><i class="ri-delete-bin-line"></i> <?php echo _("삭제") ?></a><?php } ?>
    			<?php if ($copy_href) { ?><a href="<?php echo $copy_href ?>" class="bt bt_adm" onclick="board_move(this.href); return false;"><i class="ri-file-copy-line"></i> <?php echo _("복사") ?></a><?php } ?>
    			<?php if ($move_href) { ?><a href="<?php echo $move_href ?>" class="bt bt_adm" onclick="board_move(this.href); return false;"><i class="ri-share-forward-box-line"></i> <?php echo _("이동") ?></a><?php } ?>
    			<?php if ($search_href) { ?><a href="<?php echo $search_href ?>" class="bt bt_adm"><i class="ri-search-line"></i> 검색</a><?php } ?>
    		</div>
    		<div class="button fr">
    			<a href="<?php echo $list_href ?>" class="bt bt_list"><i class="ri-menu-line"></i> <?php echo _("목록") ?></a>
...
window.addEventListener('click', function(event) {
    ...
    // 제품 360도 이미지 폴더 (KeyShotXR용, 위에서 이미 window.folderName 설정됨)
<?php if (!empty($view['wr_8'])) { ?>
if (typeof window.folderName === 'undefined') { window.folderName = "../../img360/<?php echo htmlspecialchars(preg_replace('/[^0-9a-zA-Z_.-]/','', $view['wr_8']), ENT_QUOTES); ?>"; }
<?php } ?>
