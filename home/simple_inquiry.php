...
if(isset($_POST['simple_email'])) {
    ...
    $email_subject = "[간단 문의] ".$name."님의 문의";
    ...
    $email_message = "업체명 : ".clean_string($name)."<br><br>";
    $email_message .= "이메일 : ".clean_string($email_from)."<br><br>";
    $email_message .= "전화번호 : ".clean_string($phone)."<br><br>";
    $email_message .= "문의내용 : ".clean_string(nl2br($message))."<br><br>";
...
<div id="simple_inquiry_layer" class="simple_inquiry_layer">
    <div class="simple_inquiry_content">
        ...
        <form name="simple_inquiry_form" method="post" action="/home/simple_inquiry.php" class="simple_inquiry_form">
            <div class="simple_inquiry_fields">
                <div class="field_group">
                    <label for="simple_name" class="required">업체명</label>
                    <input type="text" name="simple_name" id="simple_name" required placeholder="상담 참조용 업체명을 입력하세요. (비공개)">
                </div>
                
                <div class="field_group">
                    <label for="simple_email" class="required">이메일</label>
                    <input type="email" name="simple_email" id="simple_email" required placeholder="연락 가능한 이메일을 입력하세요.">
                </div>
                
                <div class="field_group">
                    <label for="simple_phone">전화번호</label>
                    <input type="tel" name="simple_phone" id="simple_phone" placeholder="담당자 휴대폰/전화번호 등을 입력하세요.">
                </div>
                
                <div class="field_group">
                    <label for="simple_message" class="required">문의내용</label>
                    <textarea name="simple_message" id="simple_message" required placeholder="문의 내용을 입력하세요. (최대 500자)" rows="2"></textarea>
                </div>
                ...
                <div class="field_group">
                    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap;">
                        <div style="display: flex; align-items: center;">
                            ...
                            <label for="simple_agree" style="margin: 0; display: inline;">'개인정보 수집·이용'에 동의 합니다.</label>
                        </div>
                        <a href="javascript:void(0);" onclick="togglePrivacyContent()" style="color: #1a4691; text-decoration: underline; font-size: 13px; margin-left: 10px; cursor: pointer;">개인정보 수집·이용</a>
                    </div>
            ...
            <div class="simple_inquiry_footer">
                <button type="submit" class="simple_inquiry_submit">문의하기</button>
            </div>
        </form>
    </div>
</div>
...
document.querySelector('.simple_inquiry_form').addEventListener('submit', function(e) {
    ...
    fetch('/home/simple_inquiry.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.text())
    .then(data => {
        // 서버 응답을 확인하여 성공/실패 처리
        if(data.includes('자동등록방지 숫자가 틀렸습니다')) {
            alert('자동등록방지 숫자가 틀렸습니다.\n다시 입력해주세요.');
            // 캡차 새로고침 (kcaptcha인 경우)
        ...
        } else if(data.includes('문의주셔서 감사합니다')) {
            alert('문의주셔서 감사합니다.\n빠른 시일안에 답변드리겠습니다.');
            closeSimpleInquiry();
            this.reset();
        } else {
